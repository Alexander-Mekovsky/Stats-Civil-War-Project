...
sum(is.na(df))
#The data do not have any outlier
...
```{r}
df$schooling <- sqrt(df$schooling)		 # transforming the schooling predictor
df_cleaned <- df %>%
  select(-c(dominance, civil.war)) %>%
  pivot_longer(
    cols = everything(),
...
plot <- ggplot(df_cleaned, aes(x = value,fill = variable)) + 
  geom_histogram(bins = 10) + 
	labs(title = "distribuition of Cival war Predictors") +
  facet_wrap(~ variable, scales = "free_x") +
  theme_minimal() +
  theme(
    ...
    panel.grid.minor = element_blank(), 
  	axis.text = element_text(size = 14, face = "bold"),    # Increase and bold axis text
...
# Create the plot for dominance filled by civil.war


plot2 <- ggplot(df, aes(x = dominance, fill = civil.war)) + 
  ...
  labs(
    title = "Influence of Ethnic Dominance on Civil War Occurrence", 
    x = "Dominance", 
    y = "Count",
    fill = "Civil War Status" 
  )
...
ggplot(importance_data, aes(x = MeanDecreaseAccuracy, y = reorder(Variable, MeanDecreaseAccuracy))) + 
  geom_bar(stat = "identity", size = 2, fill = "red") + 
  theme_minimal() + 
  labs(
